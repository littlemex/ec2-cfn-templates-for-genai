...
class AnrRootCauseTool(BaseTool):
    ...
    def _query_main_thread_blocks(
        ...
        proc_filter = f"AND p.name GLOB '{process_name.replace(chr(39), chr(39)*2)}'" if process_name else ""
        sql = f"""
        SELECT
          ts.ts AS ts,
          ts.dur AS dur,
    ...
    def _query_binder_delays(
        ...
        proc_filter = (
            f"AND client_process GLOB '{process_name.replace(chr(39), chr(39)*2)}'" if process_name else ""
        )
        sql = f"""
